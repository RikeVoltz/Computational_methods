/**
Лабораторная работа №3 по дисциплине "Вычислительные методы"
@name Main
...
int main() {
	...
	cout << "¬ведите начальные границы отрезков дл€ каждого из корней: " << endl;
...
void Dichotomy(root_with_borders &root) {
	while (root.borders.end - root.borders.start >= K_EPS_DICHOTOMY / 2) {
		if (f(root.borders.end)*f(root.borders.start) > 0) {
			cerr << "ќшибка в методе дихотомии" << endl;
	...
	while (fabs(x - root.root) >= K_EPS_MUELLER) {
		...
		if (f(a)*f(x) < 0)
			...
			if (f(b)*f(x) < 0)
				...
				cerr << "ќшибка в методе парабол" << endl;
				exit(MUELLER_ERROR);
			}
	}
}
...
void Verification(vector<root_with_borders>&roots) {
	for (int i = 0; i<(int)roots.size(); i++)
		if (fabs(f(roots[i].root)) > K_EPS_MUELLER) {
			cerr << "Ѕыли найдены неверные значени€ корней уравнени€" << endl;
			exit(VERIFICATION_ERROR);
		}
	cout << "Ѕыли найдены верные значени€ корней уравнени€" << endl ;
}
